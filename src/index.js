...
async function handleTranscriptRequest(params) {
  ...
  try {
    ...
    const captionTrack = await fetchCaptionTrack(videoId, supportedLanguages);
...
function handleOpenAPIRequest(request) {
  const schema = {
    ...
    info: {
      ...
      description:
        "Retrieves transcript data for YouTube videos in JSON, SRT, or plain text format.",
      version: "v1.1.0",
    },
    servers: [
      {
...
async function fetchCaptionTrack(videoId, supportedLanguages) {
  ...
  // Find the first available track in the supported languages
  let availableTrack = null;
  for (const lang of supportedLanguages) {
    availableTrack = captionTracks.find((track) =>
  ...
  if (!availableTrack)
    throw new Error("No captions found in supported languages");

  const captionResponse = await fetch(availableTrack.baseUrl);
